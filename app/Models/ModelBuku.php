...
class ModelBuku extends Model
{
    ...
    protected $allowedFields = ['id_buku','id_kategori_buku', 'id_sub_kategori', 'judul', 'penulis', 'penerbit', 'tahun_terbit', 'sinopsis', 'stok', 'sampul_buku'];
    ...
    public function edit_stok($stok_baru, $id_buku)
    {
        $builder = $this->db->table($this->table);
        $builder->where('id_buku', $id_buku);
        return $builder->update($stok_baru);
    }
    ...
    public function kode_buku() {
        ...
        if ($count == 0) {
            return 'KB-001'; // Jika tidak ada data, beri nomor otomatis pertama
        } else {
            ...
            $existingNumbers = array_map(function ($code) {
                return (int)substr($code['id_buku'], strlen('KB-'));
            }, $usedCodes);
    
            $newNumber = min(array_diff(range(1, $count + 1), $existingNumbers));
    
            $newCode = 'KB-' . sprintf("%03d", $newNumber);
