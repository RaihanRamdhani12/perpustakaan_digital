...
class ModelSubKategori extends Model
{
    ...
    protected $allowedFields = ['id_sub_kategori', 'nama_sub_kategori', 'id_kategori_buku'];
    ...
    public function kode_sub() {
        ...
        if ($count == 0) {
            return 'KS-001'; // Jika tidak ada data, beri nomor otomatis pertama
        } else {
            ...
            $existingNumbers = array_map(function ($code) {
                return (int)substr($code['id_sub_kategori'], strlen('KS-'));
            }, $usedCodes);
    
            $newNumber = min(array_diff(range(1, $count + 1), $existingNumbers));
    
            $newCode = 'KS-' . sprintf("%03d", $newNumber);
